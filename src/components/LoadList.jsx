...
const Row = ({ index, load, toggleEnabled, handleEditRow, deleteLoad, editingRowId, editedRow, handleInputChange, handleSaveRow, handleCancelEdit }) => {
  return (
    <div className="grid grid-cols-7 gap-2 items-center p-2 border-b min-w-[1000px]">
      <div className="text-center">
        <input type="checkbox" checked={load.enabled} onChange={() => toggleEnabled(index)} />
      </div>
      <div>
        {editingRowId === load.id ? (
          <input className="w-full border rounded p-1" value={editedRow.name} onChange={(e) => handleInputChange('name', e.target.value)} />
        ) : load.name}
      </div>
      <div className="text-center">
        {editingRowId === load.id ? (
          <input type="number" className="w-full border rounded p-1" value={editedRow.power} onChange={(e) => handleInputChange('power', e.target.value)} />
        ) : `${load.power} W`}
      </div>
      <div className="text-center">
        {editingRowId === load.id ? (
          <input type="number" className="w-full border rounded p-1" value={editedRow.voltage} onChange={(e) => handleInputChange('voltage', e.target.value)} />
        ) : `${load.voltage} V`}
      </div>
      <div className="text-center">
        {editingRowId === load.id ? (
          <select className="w-full border rounded p-1" value={editedRow.type} onChange={(e) => handleInputChange('type', e.target.value)}>
      ...
      <div className="text-center">
        {editingRowId === load.id ? (
          <input type="checkbox" checked={editedRow.is_motor} onChange={(e) => handleInputChange('is_motor', e.target.checked)} />
        ) : load.is_motor ? 'Yes' : 'No'}
      </div>
      <div className="flex justify-end gap-2">
        {editingRowId === load.id ? (
          <>
            <button className="inline-flex items-center gap-1 bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700" onClick={() => handleSaveRow(load.id)}>
              <FaSave /> Save
            </button>
            <button className="inline-flex items-center gap-1 bg-gray-400 text-white px-3 py-1 rounded hover:bg-gray-500" onClick={handleCancelEdit}>
              <FaTimes /> Cancel
            </button>
          </>
        ) : (
          <>
            <button className="text-blue-600 hover:text-blue-800" onClick={() => handleEditRow(load)}>
              <FaEdit />
            </button>
            <button className="text-red-600 hover:text-red-800" onClick={() => deleteLoad(load.id)}>
              <FaTrash />
            </button>
          </>
        )}
